...
class Attn(nn.Module):
    def __init__(self,):
        ...
        self.window_size = 8
        self.attn = WindowAttention(64, window_size=(8, 8), num_heads=4)
...
class VIT(nn.Module):
    def __init__(self,
        ...
        if True:
            ...
            self.net_low = ViT()
            self.net_high = ViT()
            self.up_low = Upsample(4, 64)
            self.up_high = Upsample(4, 64)
            self.conv_low = nn.Conv2d(64, 1, 3, 1, 1)
            self.conv_high = nn.Conv2d(64, 1, 3, 1, 1)
    ...
    def forward(self, x):
        if True:
            x = self.conv_first(x).view(-1, 64, 256).permute(0, 2, 1)
            x_low = self.net_low(x, self.calculate_rpi_sa()).permute(0, 2, 1).view(-1, 64, 16, 16)
            x_high = self.net_high(x, self.calculate_rpi_sa()).permute(0, 2, 1).view(-1, 64, 16, 16)
            x_low = self.conv_low(self.up_low(x_low))
            x_high = self.conv_high(self.up_high(x_high))
            x = x_low + x_high
