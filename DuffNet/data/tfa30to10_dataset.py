...
class tfa30to10Dataset(data.Dataset):

    def __init__(self, opt):
        super(tfa30to10Dataset, self).__init__()
        self.opt = opt #导入参数
        self.scale = self.opt['scale']
        self.fold = opt['use_fold']
        self.mirroring = opt['use_mirroring']
        ...
        self.river_folder = opt['dataroot_river']  # groud truth所在的文件路径
        self.paths_river = sorted(list(scandir(self.river_folder, full_path=True)))

    def random_horizontal_flip_opencv(self, img, img2, img3):
        ...
        if False: # self.fold:
            ...
            if random.uniform(0, 1) < 0.5:
                ...
                img2 = cv2.flip(img2, type_flip)
                img3 = cv2.flip(img3, type_flip)
            # 随机判断是否进行转置
            if random.uniform(0, 1) < 0.5:
                img = np.transpose(img)
                img2 = np.transpose(img2)
                img3 = np.transpose(img3)
        if True: # self.mirroring:
            # 随机判断是否进行高程镜像
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            img_gt, img_lq, sr_river = self.random_horizontal_flip_opencv(img_gt, img_lq, sr_river)
        ...
        if self.opt['phase'] == 'train':
            img_gt_low = cv2.GaussianBlur(img_gt, (5, 5), 1, 0, 1)
            ...
            img_gt_low = torch.ones(1, 48, 48)
            img_gt_high = torch.ones(1, 48, 48)
            img_gt = 2 * (img_gt - lq_min) / (lq_max - lq_min + 10) - 1
            img_lq = 2 * (img_lq - lq_min) / (lq_max - lq_min + 10) - 1
        ...
        if self.opt['phase'] == 'train':
            ...
            river_weight = torch.ones(1, 63, 63)
        # 增加维度
        img_gt = img_gt.unsqueeze(0)
        img_lq = img_lq.unsqueeze(0)

        return {'lq': img_lq, 'gt': img_gt , 'gt_path': gt_path, 'lq_path':lq_path, 'min':lq_min
            ,'max':lq_max, 'img_gt_low':img_gt_low, 'img_gt_high':img_gt_high, 'river_weight': river_weight}
